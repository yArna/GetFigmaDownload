...
class FigmaAnalyzerApp {
    ...
    setupEventListeners() {
        ...
        document.getElementById('downloadTab').addEventListener('click', () => this.switchTab('download'));
        ...
        // 下载页面筛选器事件
        document.getElementById('downloadPlatformFilter')?.addEventListener('change', () => this.applyDownloadFilters());
        document.getElementById('downloadVersionSearch')?.addEventListener('input', () => this.applyDownloadFilters());
        document.getElementById('downloadYearFilter')?.addEventListener('change', () => this.applyDownloadFilters());
        document.getElementById('downloadSortOrder')?.addEventListener('change', () => this.applyDownloadFilters());

        // 下载页面全选事件
        document.getElementById('downloadSelectAll')?.addEventListener('change', (e) => this.handleSelectAll(e.target.checked));
        document.getElementById('downloadHeaderSelectAll')?.addEventListener('change', (e) => this.handleSelectAll(e.target.checked));
    ...
    switchTab(tab) {
        ...
        if (tab === 'analysis') {
            analysisTab.className = 'tab-button border-blue-500 text-blue-600 whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm';
            downloadTab.className = 'tab-button border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm';
            
            document.getElementById('analysisContent').classList.remove('hidden');
            document.getElementById('downloadContent').classList.add('hidden');
        } else {
            downloadTab.className = 'tab-button border-blue-500 text-blue-600 whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm';
            analysisTab.className = 'tab-button border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm';
    ...
    populateAnalysisData() {
        // 总版本数
        document.getElementById('totalVersions').textContent = this.allVersions.length;
        
        // 计算平均更新间隔
        const sortedVersions = [...this.allVersions].sort((a, b) => new Date(a.date) - new Date(b.date));
        let totalDays = 0;
        let intervalCount = 0;
        ...
        const avgDays = intervalCount > 0 ? Math.round(totalDays / intervalCount) : 0;
        ...
        if (sortedVersions.length > 0) {
            ...
            const endYear = new Date(sortedVersions[sortedVersions.length - 1].date).getFullYear();
        ...
        const mostActiveYear = Object.keys(yearCounts).reduce((a, b) => 
            yearCounts[a] > yearCounts[b] ? a : b, Object.keys(yearCounts)[0]
        );
    ...
    generateInsights() {
        ...
        const mostActiveYear = Object.keys(yearCounts).find(year => yearCounts[year] === maxYear);
        
        insights.push(`最活跃的年份是 ${mostActiveYear}，共发布了 ${maxYear} 个版本`);
        
        // 分析平台分布
        const platformCounts = {};
        this.allVersions.forEach(version => {
            platformCounts[version.platform] = (platformCounts[version.platform] || 0) + 1;
        });
        ...
        insights.push(`${platformNames[topPlatform]} 平台版本最多，共 ${platformCounts[topPlatform]} 个版本`);
        
        // 分析更新趋势
        const recentYears = years.slice(-3);
        const recentAvg = recentYears.reduce((sum, year) => sum + yearCounts[year], 0) / recentYears.length;
        const overallAvg = Object.values(yearCounts).reduce((a, b) => a + b, 0) / years.length;
    ...
    updateCharts() {
        ...
        if (yearFilter !== 'all') {
            filteredData = filteredData.filter(v => new Date(v.date).getFullYear().toString() === yearFilter);
        }
    ...
    updateTimelineChart(data) {
        ...
        const datasets = platforms.map(platform => {
            const platformData = sortedMonths.map(month => 
                monthlyData[platform][month] || 0
            );
    ...
    applyDownloadFilters() {
        const platformFilter = document.getElementById('downloadPlatformFilter').value;
        const versionSearch = document.getElementById('downloadVersionSearch').value.toLowerCase();
        ...
        if (platformFilter !== 'all') {
            this.filteredVersions = this.filteredVersions.filter(version => 
                version.platforms[platformFilter]
            );
        }
    ...
    sortDownloadVersions(order) {
        switch (order) {
            case 'newest':
                this.filteredVersions.sort((a, b) => new Date(b.date) - new Date(a.date));
                break;
            case 'oldest':
                this.filteredVersions.sort((a, b) => new Date(a.date) - new Date(b.date));
                break;
            case 'version':
    ...
    updateDownloadStats() {
        document.getElementById('downloadDisplayedVersions').textContent = this.filteredVersions.length;
        
        // 计算总大小（所有平台的文件总和）
        const totalSize = this.filteredVersions.reduce((sum, version) => {
            return sum + Object.values(version.platforms).reduce((platformSum, platform) => 
                platformSum + platform.size, 0
            );
        }, 0);
        document.getElementById('downloadTotalSize').textContent = this.formatFileSize(totalSize);
    ...
    createDownloadVersionRow(version) {
        ...
        const createPlatformButton = (platform, platformData) => {
            ...
            return `
                <div class="flex flex-col items-start space-y-1">
                    <button onclick="window.figmaApp.downloadSingle('${platformData.platform}-${platformData.ver}')" 
                    ...
                    <span class="text-xs text-gray-500">${this.formatFileSize(platformData.size)}</span>
                </div>
            `;
        };
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900">${version.ver}</div>
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                ${createPlatformButton('macOS ARM', version.platforms.macOS_ARM)}
            </td>
        `;
    ...
    updateDownloadSelectedCount() {
        ...
        document.getElementById('downloadSelectedCount').textContent = `已选择 ${count} 个版本`;
        ...
        const headerSelectAllCheckbox = document.getElementById('downloadHeaderSelectAll');
    ...
    async downloadSelected() {
        ...
        this.selectedVersions.forEach(versionKey => {
            const versionGroup = this.filteredVersions.find(v => v.ver === versionKey);
            if (versionGroup) {
                // 添加该版本的所有平台
    ...
    findVersionByKey(versionKey) {
        ...
        return this.allVersions.find(v => v.platform === platform && v.ver === ver);
    }
    ...
    async startDownload(versions) {
        ...
        try {
            ...
            for (const version of versions) {
                this.updateDownloadProgress(completed, total, `正在下载 ${version.ver} (${this.getPlatformDisplayName(version.platform)})`);
                
                // 创建下载链接并触发下载
                const link = document.createElement('a');
                link.href = version.url;
                link.download = version.fileName || `${version.ver}-${version.platform}`;
            ...
            document.getElementById('modalMessage').textContent = '下载过程中出现错误: ' + error.message;
        }
    }
    ...
    updateDownloadProgress(completed, total, message) {
        ...
        document.getElementById('progressText').textContent = `${Math.round(progress)}% (${completed}/${total})`;
