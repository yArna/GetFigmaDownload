...
class FigmaDownloadManager {
    ...
    setupEventListeners() {
        // 筛选器事件
        document.getElementById('platformFilter').addEventListener('change', () => this.applyFilters());
        document.getElementById('versionSearch').addEventListener('input', () => this.applyFilters());
        ...
        document.getElementById('headerSelectAll').addEventListener('change', (e) => this.handleSelectAll(e.target.checked));
    ...
    applyFilters() {
        ...
        const versionSearch = document.getElementById('versionSearch').value.toLowerCase();
    ...
    updateStats() {
        document.getElementById('displayedVersions').textContent = this.filteredVersions.length;
        
        const totalSize = this.filteredVersions.reduce((sum, version) => sum + version.size, 0);
        ...
        if (this.filteredVersions.length > 0) {
            ...
            document.getElementById('versionRange').textContent = `${oldestVersion} - ${newestVersion}`;
        } else {
    ...
    renderVersionsTable() {
        ...
        const endIndex = Math.min(startIndex + this.itemsPerPage, this.filteredVersions.length);
    ...
    createVersionRow(version) {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${this.getPlatformBadgeClass(version.platform)}">
    ...
    updatePagination() {
        const totalPages = Math.ceil(this.filteredVersions.length / this.itemsPerPage);
        
        // 更新分页信息
        const startItem = (this.currentPage - 1) * this.itemsPerPage + 1;
        const endItem = Math.min(this.currentPage * this.itemsPerPage, this.filteredVersions.length);
        
        document.getElementById('startItem').textContent = this.filteredVersions.length === 0 ? 0 : startItem;
        document.getElementById('endItem').textContent = endItem;
        document.getElementById('totalItems').textContent = this.filteredVersions.length;

        // 更新移动端按钮
        document.getElementById('prevPageMobile').disabled = this.currentPage === 1;
        document.getElementById('nextPageMobile').disabled = this.currentPage === totalPages || totalPages === 0;
    ...
    renderPaginationControls(totalPages) {
        ...
        // 上一页按钮
        const prevButton = this.createPaginationButton('上一页', this.currentPage - 1, this.currentPage === 1);
        ...
        for (let i = startPage; i <= endPage; i++) {
            const pageButton = this.createPaginationButton(i.toString(), i, false, i === this.currentPage);
        ...
        if (endPage < totalPages) {
            ...
            const lastButton = this.createPaginationButton(totalPages.toString(), totalPages, false);
            lastButton.addEventListener('click', () => this.goToPage(totalPages));
            paginationControls.appendChild(lastButton);
        }

        // 下一页按钮
        const nextButton = this.createPaginationButton('下一页', this.currentPage + 1, this.currentPage === totalPages);
    ...
    createPaginationButton(text, page, disabled = false, current = false) {
        ...
        if (current) {
            ...
            button.className = 'relative inline-flex items-center px-4 py-2 border border-gray-300 bg-gray-100 text-sm font-medium text-gray-400 cursor-not-allowed';
        } else {
    ...
    updateSelectedCount() {
        ...
        document.getElementById('selectedCount').textContent = `已选择 ${count} 个版本`;
